...
static gfire_game_detection_set *gfire_game_config_get_dset(const gfire_game_configuration *p_gconf, const gfire_game_data *p_game_data)
{
	if(!p_gconf)
	{
		purple_debug_error("gfire", "gfire_game_config_get_dset: Invalid game config info.\n");
		return NULL;
	}
	...
	if(!game)
	{
		purple_debug_error("gfire", "gfire_game_config_get_dset: Game not existing.\n");
		return NULL;
	}
	...
	if(!game->detection_sets || !((gfire_game_detection_set*)game->detection_sets->data)->launch_args ||
	   !p_gconf->launch_file)
	{
		purple_debug_error("gfire", "gfire_game_config_get_dset: Insufficient information available!\n");
		return NULL;
	}

	// Get the first detection set & return it
	gfire_game_detection_set *dset = (gfire_game_detection_set*)game->detection_sets->data;
	return dset;
}
...
static gchar *gfire_game_config_get_command(const gfire_game_configuration *p_gconf, const gfire_game_data *p_game_data)
{
	...
	// Get first dset
	gfire_game_detection_set *dset = gfire_game_config_get_dset(p_gconf, p_game_data);
...
#ifdef _WIN32
	exe = p_gconf->launch_prefix ? g_strdup_printf("%s \"%s\"", p_gconf->launch_prefix, p_gconf->launch_file) :
		  g_strdup_printf("\"%s\"", p_gconf->launch_file);
#else
	// Wine compatibility for Linux users
	gchar *wine = "";
	if(gfire_filetype_use_wine(p_gconf->launch_file))
		wine = "wine";

	exe = p_gconf->launch_prefix ? g_strdup_printf("%s %s \"%s\"", p_gconf->launch_prefix, wine, p_gconf->launch_file) :
		  g_strdup_printf("%s \"%s\"", wine, p_gconf->launch_file);
#endif
...
void gfire_join_game(const gfire_game_data *p_game_data)
{
	...
	// Parse argv
	gchar **argv;
	if(g_shell_parse_argv(gfire_game_config_get_command(gconf, p_game_data), NULL, &argv, NULL))
	{
		g_setenv("WINEPREFIX", "/home/warren/wine", TRUE);
		g_spawn_async(g_path_get_dirname(gconf->launch_file), argv, NULL, G_SPAWN_SEARCH_PATH | G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL, NULL, NULL);
	}
}
