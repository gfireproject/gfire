...
{
	// Get parent row
    gint parent = gfire_server_brower_proto_get_parent(p_server_browser, p_server);
	if(parent == -1)
		return;
...
{
    if(!gfire_server_browser_proto_can_add_fav_server(p_server_browser))
	{
		...
		if(sscanf(ip_str, "%3hu.%3hu.%3hu.%3hu", &ip1, &ip2, &ip3, &ip4) == 4)
		{
			...
			if((ip1 < 256) || (ip2 < 256) || (ip3 < 256) || (ip4 < 256))
			{
				...
				if(sscanf(port_str, "%hu", &port) == 1)
				{
                    ...
                    gfire_server_browser_proto_add_fav_server(p_server_browser->gfire, gameid, ip_str, port_str);
                    ...
                    gfire_server_browser_proto_push_server(p_server_browser, server);
				}
			}
		}
	}
...
static void gfire_server_browser_remove_fav_server_cb(gfire_server_browser *p_server_browser, GtkWidget *p_sender)
{
	...
	if(gtk_tree_selection_get_selected(selection, &model, &iter))
	{
        ...
        if(!gfire_server_browser_proto_is_fav_server(p_server_browser, server_info->ip, server_info->port))
		{
        ...
        gfire_server_browser_proto_remove_fav_server(p_server_browser->gfire, gameid, ip_str, port_str);
...
static void gfire_server_browser_request_serverlist_cb(gfire_server_browser *p_server_browser, GtkWidget *p_sender)
{
	...
	if(gameid != 0)
	{
        ...
        gfire_server_browser_proto_set_curr_gameid(p_server_browser, gameid);
