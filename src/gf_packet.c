...
void gfire_read_avatar_info(PurpleConnection *gc, int packet_len)
{
	...
	itmp = gfire_read_attrib(&uid, gfire->buff_in + index, packet_len - index, "userid", FALSE, TRUE, 0, 0, 0);
	if (itmp < 1 ) {
//FIXME add mem cleanup code
	...
	index += itmp + 1;
	itmp = gfire_read_attrib(&atype, gfire->buff_in + index, packet_len - index, "avatartype", FALSE, TRUE, 0, 0, 0);
	if (itmp < 1 ) {
//FIXME add mem cleanup code
	...
	index += itmp + 1;
	itmp = gfire_read_attrib(&anumber, gfire->buff_in + index, packet_len - index, "avatarnumber", FALSE, TRUE, 0, 0, 0);
	if (itmp < 1 ) {
//FIXME add mem cleanup code
	...
	if (NULL == gfbl) {
		purple_debug(PURPLE_DEBUG_ERROR, "gfire", "(avatar info): uid not found in buddy list\n");
		if (NULL != atype) g_free(atype);
		return;
		if (NULL != anumber) g_free(anumber);
		return;
	}

	gbuddy = (gfire_buddy *)gfbl->data;
	if (NULL == gbuddy) {
		purple_debug(PURPLE_DEBUG_ERROR, "gfire", "(avatar info): uid found but gbuddy is {NULL}\n");
		if (NULL != atype) g_free(atype);
		return;
		if (NULL != anumber) g_free(anumber);
		return;
	...
	purple_debug(PURPLE_DEBUG_MISC, "gfire", "Got avatar info\n");

	if (NULL != gbuddy->avatartype) g_free(gbuddy->avatartype);
	if (NULL != atype) {
		gbuddy->avatartype = atype;
	} else {
		gbuddy->avatartype = NULL;
	}

	if (NULL != gbuddy->avatarnumber) g_free(gbuddy->avatarnumber);
	if (NULL != anumber) {
		gbuddy->avatarnumber = anumber;
	} else {
		gbuddy->avatarnumber = NULL;
	}
	...
	if (NULL == buddy) {
		purple_debug(PURPLE_DEBUG_ERROR, "gfire", "(avatar info): purple_buddy_find returned null for %s\n",
				NN(gbuddy->name));
		return;
	}
...
void gfire_read_join_chat(PurpleConnection *gc, int packet_len)
{
	int index = XFIRE_HEADER_LEN + 2;
	guint16 slen = 0;
	guint8 chat_id[XFIRE_CHATID_LEN];
	...
	/* join response */
	memcpy(&response, gfire->buff_in + index, 4);
	response = GUINT32_FROM_LE(response);
	if (response != 0){
		purple_notify_message(NULL, PURPLE_NOTIFY_MSG_INFO, "Groupchat", "Room denied", "Access denied, the password was wrong or no password was given.", NULL, NULL);
		return;
	}
	index += 24;
...
/**
void gfire_read_clan_buddy_list(PurpleConnection *gc, int packet_len)
{
	int index = 0;
	int itmp = 0;
	int i = 0;
	...
	GList *a,*n,*u;
	gchar uids[(XFIRE_USERID_LEN*2)+1];
	guint8 clan_id[XFIRE_CLANID_LEN];
	gfire_data *gfire = (gfire_data *)gc->proto_data;
	...
	// xfire clan id 
	memcpy(clan_id, gfire->buff_in + index, XFIRE_CLANID_LEN);
	...
	userids = g_list_first(userids);
	names = g_list_first(names);
	nicks = g_list_first(nicks);
	

	
	u = userids; n = names; a = nicks;
	while (NULL != n) {
		gf_buddy = g_malloc0(sizeof(gfire_buddy));
		gf_buddy->name = (gchar *)n->data;
		gf_buddy->alias = (gchar *)a->data;
		gf_buddy->userid = (guint8 *)u->data;
		if (NULL == gf_buddy->alias) gf_buddy->alias = g_strdup(gf_buddy->name);
		// make userid into string 
		for(i = 0;i < XFIRE_USERID_LEN; i++) g_sprintf(uids + (i*2), "%02x", gf_buddy->userid[i]);
		...
		n->data = NULL;
		u->data = NULL;
		a->data = NULL;  so we don't free stuff we don't want to 
		n = g_list_next(n);
		u = g_list_next(u);
		a = g_list_next(a);
	}
	...
	while (n != NULL) {
		gf_buddy = (gfire_buddy *)n->data;
		purple_debug(PURPLE_DEBUG_MISC, "gfire", "clan buddy info: %s, %s, %02x%02x%02x%02x, %s\n",
					NN(gf_buddy->name), NN(gf_buddy->uid_str), NNA(gf_buddy->userid, gf_buddy->userid[0]),
					NNA(gf_buddy->userid, gf_buddy->userid[1]), NNA(gf_buddy->userid, gf_buddy->userid[2]),
					NNA(gf_buddy->userid, gf_buddy->userid[3]), NN(gf_buddy->alias));
		n = g_list_next(n);
